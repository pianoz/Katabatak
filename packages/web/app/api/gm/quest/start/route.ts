...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json() as { characterId?: string; questId?: string }

  let serverRes: Response
  try {
    serverRes = await fetch(`${GM_SERVER}/gm/quest/start`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${GM_API_KEY}`,
      },
      body: JSON.stringify({ characterId: body.characterId, questId: body.questId }),
    })
  } catch {
