...
| `/dev/world-entities` | `app/dev/world-entities/page.tsx` | World entity editor — create and edit `world_entities` rows. "New Entity" button opens a modal (create mode); clicking any table row opens the same modal pre-populated (edit mode). Modal: type toggles (create only), ID prefix badge + slug input, parent combobox filtered to valid hierarchy types, auto-populated context fields, short/long description, NPC knowledge list. Table: sortable headers (ID/Name/Type/Parent), type filter toggles, parent-ID substring filter |
...
| `DELETE /api/character/[characterId]` | `app/api/character/[characterId]/route.ts` | Graceful character deletion — ownership-checked, service_role client explicitly deletes child rows across all character-scoped tables in order, then deletes the character row (remaining cascades fire as belt-and-suspenders). Returns `{ success, cleaned: { table: rowCount } }` |
...
| File | Purpose |
|------|---------|
| `components/devtools-section.tsx` | Admin panel links (requires `is_dev`): Skill Tree, Items, Spells, Active Skills, Users, World Entities (plain-bordered); Prompt Evaluator, Prompt Builder, Test Suite, Logs (cyan); Combat Test (destructive red) |
...
|---------|----------------|
| `character-service.ts` | Character CRUD, stat updates, skill/spell/inventory operations. `deleteCharacter(characterId)` calls `DELETE /api/character/[characterId]` (server-side route handles cascading cleanup) |
...
| `world-entity-service.ts` | `world_entities` CRUD — `listWorldEntities`, `getWorldEntityById`, `getEntitiesByTypes`, `createWorldEntity`, `updateWorldEntity` (patches name/parent/context/data, sets `updated_at` explicitly). Never writes `search_vector` (GENERATED ALWAYS) |
...
| `bumper-lanes.ts` | Client-side copy of the normalization maps from `packages/server/gm/bumper-lanes.ts` (LEDGER_ACTIONS, LORE_ACTION_TYPES, LORE_POOLS, QUEST_STATUSES, ITEM_TYPES). Keep in sync with server when adding new aliases. Server-only: `NPC_MUTATION_FIELDS` (normalizes `update_npc` mutation key names e.g. `addknowledge` → `knowledge_append`) — not needed client-side |
...
│   ├── world-service.ts           # searchWorldEntities, getCampaignFacts, getNpcsForGame, getNpcsForCharacter, getNpcByWorldEntityId, spawnNpcInstanceFromWorldEntity. `updateNpcMutations` accepts optional `gameId`; if the npc UUID isn't found, falls back to treating it as a world_entity ID — finds or spawns a per-game instance before applying mutations. Handles `knowledge_append` into `data.knowledge`. Conversation buffer API: `decrementNpcBuffers(gameId?, characterId?)` — bleeds buffer_count before hydration; `applyParticipantBuffers(referents, candidates, gameId?)` — resolves NL referents, find/spawns instances, sets buffer_count=5; `clearNpcBuffers(gameId?, characterId?)` — zeroes all buffers on location change
...
└── gm/
    ├── handler.ts                 # Pipeline orchestrator — per-stage wall-clock timing (hydrator/lore/architect) populated into _timingOut ref after architect stream closes. Threads requestId (from GMMessageInput) into all synLog calls and every stage invocation. NPC buffer steps: `decrementNpcBuffers()` before autoHydrate; `applyParticipantBuffers()` + optional `hydrateNpcs()` re-hydration after lore engine
    ...
    ├── auto-hydrator.ts           # Layer 1: builds ContextBlock by composing 5 exported module functions. Exported modules: `hydrateCharacter` (stats, pool texts, quest notes), `hydrateInventory` (tracked+equipped items only, carry weight), `hydrateGame` (syngemGame, multiplayer game, encounter), `hydrateNpcs` (enriched+filtered NPC list — queries `npcs` table first, then falls back to `world_entities` type='npc' at the character's location; deduplicates via `world_entity_id`; spawning happens lazily on first interaction, not at hydration time), `hydrateLocation` (chain walk-up + entities physically at current place + connected sibling locations + improvised entities). `enrichAndFilterNpcs` resolves descriptions from `npc.data.long_description`/`short_description` first, falling back to `personality_profile.personality`/`small_summary`; knowledge is gated on `buffer_count > 0`. `autoHydrate` composes all 5 in two parallel batches. `contextBlock()`, `resolveLocationEntities()`, and `hydrateNpcs` remain exported (handler re-calls `hydrateNpcs` after applying participant buffers)
    ├── state-executor.ts          # Layer 5b: validates and applies Ledger output to DB. create_entity deduplicates against world_entities then improvised_entities; grant_item writes to character_inventory and sets `tracked=true` for quest-type or special/rare/epic/legendary rarity items; update_npc passes gameId to updateNpcMutations enabling lazy spawn of world_entity NPC instances; move_character additionally calls `clearNpcBuffers()` to evict NPC knowledge on scene change. All internal helpers accept requestId for trace logging
    ├── agents/
    │   ├── lore-engine.ts         # Layer 2: intent + mechanics (Haiku, slug: lore-engine). Validates LLM JSON against LoreEngineOutputSchema (safeParse); logs Zod issues on failure and returns no-check task fallback. Now emits `participant_buffers?: string[]` — NL referents of NPCs being spoken to this turn. Accepts requestId for trace logging
...
| `npcs` | NPC instances scoped to a character. `character_id` FK → `characters(id) ON DELETE CASCADE` — ownership key; character deletion cascades to all their NPC instances. `game_id` nullable legacy FK (companion NPCs have `game_id = null`). `following_character_id` SET NULL FK — companion relationship (clears when character deleted, but ownership is tracked via `character_id`). `world_entity_id` FK to `world_entities` — set when spawned from a canonical world NPC; null for hand-crafted rows. `data` JSONB mirrors `world_entities.data`: `{ short_description, long_description, knowledge[] }`. `buffer_count` int (default 0) — knowledge is only injected into GM context when `> 0`; set to 5 on conversation detection, decremented 1/turn, zeroed on location change |
...
| `world_entities` | World encyclopedia. Replaced `world_lore`. `type` enum (`nation`\|`region`\|`place`\|`location`\|`npc`\|`item`), `data` JSONB (incl. `short_description`, `long_description`, `knowledge[]` for NPC entities), full-text `search_vector`. Rows of `type = 'npc'` are the canonical base state; per-character instances live in `npcs` via `world_entity_id` |
| `character_entity_mutations` | Per-character overrides on `world_entities` (e.g. hidden doors, altered descriptions). `character_id` FK → `characters(id) ON DELETE CASCADE`. Written by the GM server (`delete_entity` action); players never write directly |
...
| `20260524220000_replace_world_lore_with_entities.sql` | Replaces `world_lore` with `world_entities` + `player_entity_mutations` (later renamed to `character_entity_mutations`) |
...
| `20260612000000_add_world_entity_ref_and_data_to_npcs.sql` | Adds `world_entity_id VARCHAR(64)` FK → `world_entities(id)` and `data JSONB` column to `npcs`. Indexes on `world_entity_id`. Backfills `data` from existing `small_summary` (→ `short_description`) and `personality_profile.personality` (→ `long_description`) |
| `20260613000000_add_buffer_count_to_npcs.sql` | Adds `buffer_count INT NOT NULL DEFAULT 0` to `npcs`. Gates knowledge tier hydration — knowledge is injected into GM context only while buffer > 0 |
| `20260612100000_rename_player_mutations_to_character.sql` | Renames `player_entity_mutations` → `character_entity_mutations`; renames `player_id` → `character_id`; adds `character_id FK → characters(id) ON DELETE CASCADE`; fixes broken RLS policy (old policy compared character UUIDs against `auth.uid()`) |
| `20260612110000_add_character_id_to_npcs.sql` | Adds `character_id uuid FK → characters(id) ON DELETE CASCADE` to `npcs`; backfills from `following_character_id` (companions) and `game_members` join (game-owned NPCs) |
