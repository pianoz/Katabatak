...
export function ActionCard({
  ...
  const itemSelect = (
    <select
      ...
      className="bg-secondary/40 text-[10px] uppercase tracking-wider text-foreground border border-border h-9 px-2 rounded-sm focus:ring-1 focus:ring-foreground/20 w-full cursor-pointer"
    >
    ...
    return (
      <div className="border border-border bg-card p-3 flex flex-col gap-3">
        ...
        <div className="border-t border-border pt-3 flex flex-col sm:flex-row gap-4 items-stretch">
          {/* Weak option: button stacked above its effect/cost */}
          ...
          {/* Strong option: button stacked above its effect/cost */}
          <div className="flex flex-col items-center gap-2 flex-1">
            <Button
          ...
          {/* Description + Condition pushed to the far right */}
          {selectedItem && (
            <div className="flex items-center gap-3 sm:max-w-[35%]">
  ...
  // ── Single layout (original, no strong variant) ────────────────────────────
