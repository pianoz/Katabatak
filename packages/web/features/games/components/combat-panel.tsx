...
import { getEncounterCreatures, updateEncounterCreature, removeEncounterCreature } from "@/lib/services/encounter-service"
...
export function CombatPanel({ gameId, refreshKey }: CombatPanelProps) {
  ...
  useEffect(() => {
    setLoading(true)
    getEncounterCreatures(createClient(), gameId).then((data) => {
      setCreatures(data as EncounterCreature[])
      setLoading(false)
    })
  }, [gameId, refreshKey])
  ...
  async function spendPool(id: string, pool: "power" | "will" | "essence", cost: number) {
    ...
    await updateEncounterCreature(createClient(), id, { [field]: newVal })
  }
  ...
  async function handleHpChange(id: string, delta: number) {
    ...
    await updateEncounterCreature(createClient(), id, { current_health: newHp, is_alive: isAlive })
    setCreatures((prev) =>
  ...
  async function handleRemove(id: string) {
    ...
    await removeEncounterCreature(createClient(), id)
  }
              ...
              >
                ...
                {c.health_max != null && c.health_max > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => handleHpChange(c.id, +1)}
                      disabled={!c.is_alive && c.current_health === 0}
                      className={ghostBtnClass}
                    >
                      +
                    </button>
