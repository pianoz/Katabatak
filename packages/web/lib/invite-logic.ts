 ...
 * Requires the unique constraint game_members_game_profile_unique in the DB.
 */
export async function invitePlayer(
  ...
  return supabase.from("game_members").upsert(
    { game_id: gameId, profile_id: profileId, character_id: null, role: "player", member_status: "invited" },
 ...
 * This allows the GM to re-invite the player later without duplicate rows.
 */
 ...
 * Deleting the row (rather than nulling character_id) allows the GM to re-invite them.
 */
