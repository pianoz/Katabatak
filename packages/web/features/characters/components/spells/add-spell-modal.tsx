...
import { getAllSpells } from "@/lib/services/spell-service"
import { addSpellsToCharacter } from "@/lib/services/skill-service"
...
export function AddSpellModal({ characterId }: AddSpellModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      getAllSpells(supabase).then((data) => setCatalog(data as Spell[]))
    } else {
  ...
  const handleAddSelected = async () => {
    ...
    const spellsToAdd = catalog
      ...
      .map((spell) => ({ character_id: characterId, spell_id: spell.id }))

    const { error } = await addSpellsToCharacter(supabase, spellsToAdd)

    if (!error) {
      setIsOpen(false)
      router.refresh()
    } else {
      console.error("Error adding spells:", error.message)
    }
    setLoading(false)
  }
