...
import { getCatalogItems, addItemsToInventory } from "@/lib/services/item-service"
...
export function AddItemModal({ characterId, type = "all" }: AddItemModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      getCatalogItems(supabase, type).then((data) => setCatalog(data as Item[]))
    } else {
  ...
  const handleAddSelected = async () => {
    ...
    const itemsToAdd = catalog
      ...
      .map((item) => ({
        ...
        item_id: item.id,
        condition: (item as Item & { default_condition?: number }).default_condition ?? 100,
      }))

    const { error } = await addItemsToInventory(supabase, itemsToAdd)
